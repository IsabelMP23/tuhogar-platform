...
const getAllProperties = async (req, res) => {
  try {
    ...
    const { data, count } = await propiedadModel.getPropertiesPaginated(page, limit);
    res.json({ page, limit, total: count, totalPage: Math.ceil(count / limit), data });
  } catch (error) {
...
const getPropertyDetailsById = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Error al obtener los detalles de la propiedad" });
  }
};
